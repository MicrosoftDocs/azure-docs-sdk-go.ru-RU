...
ms.locfileid: "32319889"
---
# <a name="authentication-methods-in-the-azure-sdk-for-go"></a>Методы аутентификации в пакете Azure SDK для Go

Пакет Azure SDK для Go предлагает множество типов и методов аутентификации, которые можно использовать в приложении. Поддерживаются разные методы аутентификации, начиная от извлечения данных из переменных среды и заканчивая интерактивной веб-аутентификацией. В этой статье описаны типы аутентификации, доступные в пакете SDK, а также методы их использования. Кроме того, здесь приведены рекомендации по выбору типа аутентификации для конкретного приложения.

## <a name="available-authentication-types-and-methods"></a>Доступные типы и методы аутентификации

В пакете Azure SDK для Go предоставляется несколько типов аутентификации с применением разных наборов учетных данных. Для каждого из таких типов используются разные методы аутентификации, определяющие, каким образом пакет SDK принимает эти учетные данные в качестве входных. В приведенной ниже таблице описаны доступные типы аутентификации и ситуации, в которых их рекомендуется использовать в приложении.
...
| Учетные данные клиента | У вас есть субъект-служба, настроенный для этого приложения или класса, к которому оно относится. Дополнительные сведения см. в статье [Создание субъекта-службы Azure с помощью Azure CLI 2.0]. |
...
| Маркер устройства | Приложение предназначено __только__ для интерактивного использования широким кругом пользователей, возможно, из нескольких клиентов AAD. При этом пользователи имеют доступ к веб-браузеру для входа. Дополнительные сведения см. в разделе об [использовании аутентификации на основе маркера устройства](#use-device-token-authentication).|
| Имя пользователя или пароль | У вас есть интерактивное приложение, в котором нельзя использовать другой метод аутентификации. При этом для входа AAD пользователей не включена аутентификация MFA. |
...
> [!NOTE]
> Если нет особых требований, избегайте проверки пароля или имени пользователя. Если требуется вход на основе учетных данных пользователя, обычно можно использовать вместо этого аутентификацию на основе маркера устройства.
...
Для этих типов аутентификации используются разные методы. При [_аутентификации на основе среды_](#use-environment-based-authentication) учетные данные считываются непосредственно из среды программы. При [_аутентификации на основе файла_](#use-file-based-authentication) загружается файл, содержащий учетные данные субъекта-службы. При [_аутентификации на основе клиента_](#use-an-authentication-client) используется объект в коде Go. При этом вы отвечаете за предоставление учетных данных во время выполнения программы. Наконец, при [_аутентификации на основе маркера устройства_](#use-device-token-authentication) пользователь должен выполнить интерактивный вход через веб-браузер с использованием маркера. При этом нельзя также использовать аутентификацию на основе среды или файлов.
...
Если приложение работает в жестко контролируемой среде, например в контейнере, аутентификация на основе среды — идеальный вариант. Перед запуском приложения настраивается среда оболочки. Во время выполнения пакет SDK считывает переменные среды для аутентификации в Azure. 

Аутентификацию на основе среды поддерживают все методы, за исключением аутентификации на основе маркера устройства. Приоритет определяется в следующем порядке: учетные данные клиента, сертификаты, имя пользователя или пароль и функция "Управляемое удостоверение службы" (MSI). Если требуемая переменная не определена или пакет SDK получает сообщение об отказе от службы аутентификации, выполняется попытка применить следующий тип аутентификации. Если пакет SDK не может выполнить аутентификацию из среды, возвращается ошибка.
...
| __MSI__ | | Для MSI не нужно указывать никаких учетных данных. Приложение должно выполняться в ресурсе Azure, настроенном для использования MSI. Дополнительные сведения см. в статье [Управляемое удостоверение службы (MSI) для ресурсов Azure]. |

Если нужно подключиться к облаку или конечной точке управления, отличной от общедоступного облака Azure по умолчанию, также можно задать указанные ниже переменные среды. Как правило, их задают при использовании Azure Stack, облака, находящегося в другой географической области, или классической модели развертывания Azure.

| Переменная среды | ОПИСАНИЕ  |
|----------------------|--------------|
| `AZURE_ENVIRONMENT` | Имя облачной среды, к которой нужно подключиться. |
| `AZURE_AD_RESOURCE` | Идентификатор ресурса Active Directory, который нужно использовать при подключении. Это должен быть URI, указывающий на вашу конечную точку управления. |
...
Аутентификации на основе файла работает с учетными данными клиента, только если они хранятся в формате локального файла, созданного [Azure CLI 2.0](/cli/azure). Этот файл можно легко создать при создании субъекта-службы с использованием параметра `--sdk-auth`. Если вы планируете применить аутентификацию на основе файлов, убедитесь, что этот аргумент задан при создании субъекта-службы. Так как CLI выводит выходные данные в `stdout`, перенаправьте их в файл.
...
Дополнительные сведения об использовании субъектов-служб и управлении правами доступа к ним см. в статье [Создание субъекта-службы Azure с помощью Azure CLI 2.0].

## <a name="use-device-token-authentication"></a>Использование аутентификации на основе маркера устройства

Если нужно обеспечить интерактивный вход пользователей, лучший вариант – это аутентификация на основе маркера устройства. В процессе такой аутентификации пользователю передается маркер, который нужно вставить на веб-сайте входа Майкрософт, где выполняется вход в систему с учетной записью Azure Active Directory (AAD). Этот метод аутентификации поддерживает учетные записи с включенной аутентификацией MFA, в отличие от стандартного метода проверки имени пользователя или пароля.
...
Создайте структуру аутентификации с использованием соответствующей функции `New` и вызовите в полученном объекте `Authorize`, чтобы выполнить аутентификацию. Например, для аутентификации на основе сертификатов:
